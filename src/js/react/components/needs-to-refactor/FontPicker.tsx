import UseBreadcrumbs, {
    UseBreadcrumbsInjectedProps,
} from "app-react/HOC/UseBreadcrumbs"
...
import {
    ...
    ButtonSpec,
    BackgroundImage,
} from "models/ThemeSetting"
import {
    GetKioskThemeRequestParameters,
    getKioskTheme,
    createUpdateKioskTheme,
    KioskThemeCreateUpdateRequestParameters,
} from "services/backend/kiosktheme"
...
import FontPickerWithVariants, {
    FontWithVariants,
} from "app-react/components/container/FontPickerWithVariants"
import ShapedButton, {
    ButtonShape,
} from "app-react/components/presentational/ShapedButton"
...
type MapStateProps = RouteComponentProps<any> &
    Props &
    MappedProps &
    MappedDispatch
...
class ThemingRoot extends React.PureComponent<StyledProps, State> {
    ...
    protected changeBackgroundType(type: BackgroundType) {
        const bgSetting = this.state.themeSetting[
            ThemeSettingIdentifier.setting_bg
        ]
        this.cached[bgSetting.type] = this.state.themeSetting[
            ThemeSettingIdentifier.setting_bg
        ].value
        if (type === BackgroundType.solidColor) {
            ...
            this.changeBackground({
                type: type,
                value: defaultTo<GradientColor, GradientColor>(
                    this.cached[type],
                    {
                        colorStops: [],
            ...
            this.changeBackground({
                type: type,
                value: defaultTo<ImageBackground, ImageBackground>(
                    this.cached[type],
                    {
                        file: {
                            full_url: null,
    ...
    protected uploadImageResources(
        type: KioskResourceType.BackgroundImage | KioskResourceType.LogoImage,
        files: File[]
    ) {
        const kioskId = this.getKioskId()
        const resources = [...Array(files.length).keys()].map(
            (index): KioskResource => {
                return {
                    kiosk_id: kioskId,
                    resource_type: type,
    ...
    componentDidUpdate(prevProps: StyledProps) {
        if (
            prevProps.resourceCreatingUpdate !==
                this.props.resourceCreatingUpdate &&
            !this.props.resourceCreatingUpdate
        ) {
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <PageInfo
                    title={"Theming"}
                    explanation={<Typography>TODO</Typography>}
                />
                <div className={classes.createButtonContainer}>
                    <AppButton
                        disabled={this.props.themeSaving}
                        onClick={() =>
                            this.props.saveKioskTheme({
                                ...
                                setting: {
                                    ...
                                    setting_general: {
                                        ...
                                        textColor: this.state.themeSetting[
                                            ThemeSettingIdentifier
                                                .setting_button
                                        ].primary.background,
                                    },
                ...
                <AttachedTabs
                    ...
                    onChange={(e, index) => this.setState({ activeTab: index })}
                >
                ...
                >
                    <FontPicker
                        api_key={SiteConfig.googleFontApiKey}
                        variants={{ bold: true, italic: true }}
                        font={{
                            family: "Lato",
                        }}
                        onChange={f => {}}
...
const mapStateToProps = (
    { frontend, data }: AppStore,
    props: MapStateProps
): MappedProps => {
    const resources: KioskResource[] = Object.keys(data.kioskResources).map(
        id => data.kioskResources[id]
    )
    const kioskId = props.match.params["kioskId"]
    return {
        logoImageResources: resources.filter(
            res => res.resource_type === KioskResourceType.LogoImage
        ),
        themeSetting: defaultTo<KioskThemeSetting>(data.kioskThemes[kioskId], {
            kiosk_id: null,
            setting: null,
        }).setting,
        backgroundImageResources: resources.filter(
            res => res.resource_type === KioskResourceType.BackgroundImage
        ),
...
const mapDispatchToProps = (
    ...
    return {
        saveKioskTheme: (params: KioskThemeCreateUpdateRequestParameters) =>
            dispatch(createUpdateKioskTheme(params)),
        getKioskTheme: (params: GetKioskThemeRequestParameters) =>
            dispatch(getKioskTheme(params)),
        loadKioskResoruces: (params: KioskResourceIndexRequestParameters) =>
            dispatch(getKioskResources(params)),
        createKioskResource: (params: KioskResourceCreateRequestParameters) =>
            dispatch(createKioskResource(params)),
...
const Preview = decorator(
    class extends React.PureComponent<
        PreviewProps & WithStyles<typeof styles>,
        never
    > {
        render() {
        ...
        protected deleteResource(data: ImageResource) {
            ...
            deleteKioskResource(
                this.props.resources.find(
                    res => res.data.upload_rel === data.upload_rel
                ).id
            )
        }

        protected backgroundTypeChanged(
            ...
            return (
                <div className={classes.tabContent}>
                    <div className={classes.tabContentHeader}>
                        <Typography
                            variant={"h6"}
                            className={classes.tabContentTitle}
                        >
                            Kiosk Background
                        </Typography>
                        <Paper elevation={5}>
                            ...
                            >
                                <MenuItem value={BackgroundType.image}>
                                    Photo/Image
                                </MenuItem>
                                <MenuItem value={BackgroundType.solidColor}>
                                    Solid Color
                                </MenuItem>
                                <MenuItem value={BackgroundType.gradientColor}>
                                    Gradient Color
                                </MenuItem>
                            </Select>
                        </Paper>
                            ...
                            <div className={classes.imagesContainer}>
                                {this.props.resources.map(imageRes => (
                                    <SelectableImage
                                        ...
                                        selected={
                                            imageRes.data.upload_rel ===
                                            backgroundSetting.value.file
                                                .upload_rel
                                        }
                        ...
                        <div className={classes.imagesContainer}>
                            {this.props.resources.map(imageRes => (
                                <SelectableImage
                                    ...
                                    selected={
                                        imageRes.data.upload_rel ===
                                        logoSetting.file.upload_rel
                                    }
                                    identifier={imageRes.data}
                                    onDelete={this.deleteResource}
                                    onSelection={this.logoSelected}
                                    imageProps={{
                                        src: imageRes.data.full_url,
                                    }}
                                />
                        ...
                        <Grid container>
                            <Grid item md={4}>
                                <div className={classes.uploadSection}>
                                    ...
                                    <FileDropzone
                                        disabled={resourceCreatingUpdate}
                                        onDropAccepted={this.dropAccepted}
                                        classes={{
...
const ButtonSpecControl = decorator(
    class extends React.PureComponent<ButtonSpecProps & StyleProps> {
        render() {
            ...
            const shapeVariants = Object.keys(ButtonShape).map(
                key => ButtonShape[key]
            )
            return (
                <React.Fragment>
...
export default withRouter(
    connect(
        mapStateToProps,
        mapDispatchToProps
    )(UseBreadcrumbs(decorator(ThemingRoot)))
)
