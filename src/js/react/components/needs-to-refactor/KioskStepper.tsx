...
const styles = (theme: Theme) => {
    const breadcrumbSetting =
        theme.custom.kiosk.setting[ThemeSettingIdentifier.setting_general]
            .breadcrumb
    const activeBreadcrumbSetting =
        theme.custom.kiosk.setting[ThemeSettingIdentifier.setting_general]
            .activeBreadcrumb
        ...
        simpleIcon: {
            ...
            "&$active": {
                backgroundColor: toRGBAString(
                    activeBreadcrumbSetting.background
                ),
...
interface KioskStepperProps
    extends StandardProps<{}, StyleClassKeys<typeof styles>> {
...
function KioskStepper({ breadcrumbs, classes }: KioskStepperProps) {
    const steps: StepSpec[] = [
        {
            content: (
                <HomeIcon
                    className={classNames(classes.simpleIcon, {
    ...
    return (
        <Stepper
            connector={null}
            activeStep={breadcrumbs.length - 1}
            nonLinear
            classes={{
                    ...
                    <React.Fragment key={index}>
                        <Step completed={false}>
                            <StepButton disableRipple icon={""} {...extraProps}>
                                <StepIcon
                                    active={index < breadcrumbs.length}
                                    icon={stepSpec.content}
                                    classes={{
