...
import RouteProvider, {
    adminRouteProvider,
    AdminRoutingContext,
} from "routing/RoutesProvider"
import { IClient } from "models/Client"
import { ActiveLocationInjectedProps } from "app-react/HOC/WithActiveLocation"
import withRoutingContext, {
    WithRoutingContextInjectedProps,
} from "app-react/HOC/WithRoutingContext"
import RoleLevel from "models/RoleLevel"
import withLoggedInUser, {
    WithLoggedInUserInjectedProps,
} from "app-react/HOC/WithLoggedInUser"
import { RouteComponentProps, withRouter, Link } from "react-router-dom"
import UseBreadcrumbs, {
    UseBreadcrumbsInjectedProps,
} from "app-react/HOC/UseBreadcrumbs"
import withLocationVisitorTypes, {
    WithLocationVisitorTypesInjectedProps,
} from "app-react/HOC/WithLocationVisitorTypes"
...
type StyledProps = BackendSidebarProps &
    WithStyles<typeof styles> &
    RouteComponentProps<any>

interface BackendSidebarProps
    extends WithLoggedInUserInjectedProps,
        DrawerProps,
        ActiveLocationInjectedProps,
        WithRoutingContextInjectedProps,
        WithLocationVisitorTypesInjectedProps {
    logoutClicked: () => void
    clients: IClient[]
...
class BackendSidebar extends React.PureComponent<StyledProps, State> {
    ...
    protected buildSidebarItems(item: ISidebarItem): ReactNode {
        const childs: ReactNode =
            item.hasChilds && item.childs ? (
                <List>
                    {item.childs.map(item => this.buildSidebarItems(item))}
                </List>
            ) : null
    ...
    protected getLocationRouteProvider() {
        return this.props.locationRoutingContext
            ? RouteProvider(this.props.locationRoutingContext)
            : RouteProvider(new AdminRoutingContext())
    }
            ...
            locationItems = [
                {
                    ...
                    childs: [
                        ...
                        },
                        {
                            ...
                            title: "Visitor Types",
                            hasChilds: this.props.allVisitorTypes
                                ? this.props.allVisitorTypes.length > 0
                                : false,
                            childs: this.props.allVisitorTypes
                                ? this.props.allVisitorTypes.map(
                                      visitorType => {
                                          const item: ISidebarItem = {
                                              ...
                                              hasChilds: false,
                                              route: provider.react.backend.visitorSetting.visitorTypes.visitorTypeSetting(
                                                  visitorType.id.toString()
                                              ),
                                          }
                                          return item
                                      }
        ...
        let sideBarItems: ISidebarItem[] = [
            {
                ...
                childs: this.props.clients.map(client => {
                    const item: ISidebarItem = {
                        ...
                        route: adminRouteProvider.react.backend.locations({
        ...
        return {
            ...
            childs:
                item.expanded && item.hasChilds && item.childs
                    ? item.childs.map(i => this.setCommonItemProps(i))
                    : item.childs,
        }
    }
        ...
        return (
            <React.Fragment>
                ...
                <List className={classes.navListDynamic}>
                    {items.map(item => this.buildSidebarItems(item))}
                </List>
            </React.Fragment>
        )
    }
            ...
            <List className={classes.userNavList}>
                ...
                >
                    <UserPresenter
                        ...
                        imageUrl={props.loggedInUser.image_url}
                        classes={{
                ...
                <Collapse
                    in={this.getStateForItem("user-actions").expanded}
                    unmountOnExit={true}
                >
                    <List>
                        <ListItem
                            {...{
                                to: userHasAccess(this.props.loggedInUser, {
                                    ...
                                    ? adminRouteProvider.react.backend.users()
                                    : this.getLocationRouteProvider().react.backend.clientSetting.userAccess.index(),
                            }}
                            component={Link}
                            key={"user-settings"}
                            button
...
const mapStateToProps = (state: AppStore) => ({
    clients: state.frontend.clientsIndex.clients
        ? state.frontend.clientsIndex.clients.map(
              index => state.data.clients[index]
          )
        : [],
})
...
export default withRouter(
    connect(
        ...
        withLocationVisitorTypes(
            withRoutingContext(
                withLoggedInUser(withActiveLocation(decorator(BackendSidebar)))
            )
        )
    )
)
