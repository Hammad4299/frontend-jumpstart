import {Theme, WithStyles, ListItemIcon, ListItemAvatar, IconButton, PropInjector} from "@material-ui/core";
...
const x:StylesType<StyleClassKeys<typeof styles>|ListItemClassKey> = styles as any;
...
function SidebarItem(props:StyledSidebarItemProps)  {
    const {childs, active, expandClicked=()=>{},classes,children,expanded,icon,hasChilds,identifier,route = null,itemClicked = ()=>{},loaded,accessRequirement,...rest} = props;
    const other = {
        to: route,
    };
    ...
    return (
        <React.Fragment>
            <ListItem className={classNames(classes.root,{
                [classes.active]: active
            })} {...other} component={route ? NavLink : 'li'} dense {...rest} onClick={()=>itemClicked ? itemClicked(props) : null}>
                <div className={classes.itemContent}>
                    {icon}
                    <ListItemText title={props.title}>{props.title}</ListItemText>
                    {hasChilds ? (
                        <IconButton onClick={e=>expandClicked(e,props)} style={{
                            padding: '0px'
                        }}>
