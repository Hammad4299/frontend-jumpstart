...
import {Theme, WithStyles, List, ListItem, ListItemIcon} from "@material-ui/core";
...
function Breadcrumbs({classes, breadcrumbs, ...rest}:StyledProps) {
    const items = breadcrumbs.map((item,index)=>(
        <ListItem className={classes.item} key={item.type} {...{to:item.route}} component={Link} title={item.name}>
            {index===0 ? 
        ...
        if(nodes.length>0){
            nodes.push(<ListItem className={classes.slash} key={`separate${nodes.length}`}> / </ListItem>);
        }
        nodes.push(current);
        return nodes;
    },[]);
