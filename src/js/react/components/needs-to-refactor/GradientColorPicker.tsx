...
import { Theme, createStyles, withStyles, WithStyles, ClickAwayListener } from '@material-ui/core';
import CombinationInput, { CombinationInputValue } from './CombinationInput';
import { GradientType, GradientColor, GradientStop, UnitValue, RadialGradientPosition, RadialGradientShape, RadialGradientSize, RadialGradientSizeType, isEllipseSize, LinearGradientParams, RadialGradientParam, EllipseSize, CircleSize } from 'models/Gradient';
...
export function GradientTypePicker ({value, onChange = ()=>{}, ...rest}:GradientTypePickerProps) {
    ...
    return (
        <AppSelect options={types} isSearchable={false} value={{
            value: value,
            label: value
        }} onChange={(e:OptionType)=>onChange(e.value as GradientType)} {...rest} />
    )
}
...
export function RadialShapePicker ({value = RadialGradientShape.auto, onChange = ()=>{}, ...rest}:RadialShapePickerProps) {
    const types:OptionType[] = [{
    ...
    return (
        <AppSelect options={types} isSearchable={false} value={{
            value: value,
            label: value
        }} onChange={(e:OptionType)=>onChange(e.value as RadialGradientShape)} {...rest} />
    )
}
...
function AngleSelector(props:AngleSelectorProps) {
    ...
    return (
        <CombinationInput classes={{
                ...
                label: 'deg', value: 'deg'
            }]} onChange={(val:CombinationInputValue<OptionType>)=>onChange({
...
function RadialSizeInput(props:RadialSizeProps) {
    const fixedUnits  = Object.keys(RadialGradientSizeType).map(key=>RadialGradientSizeType[key]);
    const mapToOption = (arr:string[])=>arr.map(item=>({
        label: item,
        value: item
    }));
    ...
    const radiusInputDisabled = fixedUnits.indexOf(value.radius.unit)!==-1;
    ...
    return (
        <React.Fragment>
            <CombinationInput classes={{
                ...
                selection: radiusUnits.find(u=>u.value===value.radius.unit)
            }} inputProps={{
                placeholder: 'Radius',
            ...
            {props.shape === RadialGradientShape.ellipse && isEllipseSize(value) && (
                <CombinationInput classes={{
                    root: props.className
                }} dropdownProps={{
                    isSearchable: false
                    ...
                    selection: lengthUnits.find(u=>u.value===value.length.unit)
                }} inputProps={{
                    placeholder: 'Length',
                    style: {
                        width: '75px'
...
class Picker extends React.PureComponent<StyledProps,State> {
    ...
    protected valueChanged() {
        const { onChange = ()=>{} } = this.props;
        let a = this.getColor();
        a.colorStops = this.grapick.getHandlers().map((handler:any):GradientStop=>{
            return {
                position: {
                    unit: '%',
    ...
    protected initColorStops(stops:GradientStop[]) {
        if(this.grapick) {
            this.grapick.clear();
            stops.forEach(stop=>{
                this.grapick.addHandler(stop.position.value, toRGBAString(stop.color))
            })
        }
    }
    ...
    render() {
        ...
        return (
            <div className={classes.root}>
                <ClickAwayListener onClickAway={()=>this.grapick.getSelected() ? this.grapick.getSelected().deselect() : null}>
                    <div>
                        {this.state.handlerToPickColor && (
                                <div className={classes.colorPicker} style={{left: `calc(${this.state.handlerToPickColor.getPosition()}% - 100px)`}}>
                                    <SolidColorPicker 
                                        color={parseRGBAString(this.state.handlerToPickColor.getColor())} 
                                        onChangeComplete={(color)=>this.state.handlerToPickColor.setColor(toRGBAString(color.rgb))}  />
                                </div>
                        )}
                        <div className={classes.colorStopPicker} ref={this.pickerElementRef}></div>
                    </div>
                </ClickAwayListener>
                <div className={ classes.controlsSection }>
                    <GradientTypePicker className={classes.margin} value={color.type} onChange={this.gradientTypeChanged} />
                    {color.type === GradientType.linear && 
                        <AngleSelector className={classes.margin} value={color.param.angle} onChange={(v)=>this.angleChanged(v.value)} />
                    }
                    {color.type === GradientType.radial && (
                        <React.Fragment>
                            <RadialShapePicker className={classes.margin} value={color.param.shape} onChange={this.radialShapedChanged} />
                            <div className={classes.controlsSection}>
                                <RadialSizeInput shape={color.param.shape} className={classes.margin} value={color.param.size} onChange={(val)=>this.radialSizeChanged(val)} />
                            </div>
                            <div className={classes.controlsSection}>
                                <RadialPositionInput className={classes.margin} value={color.param.position} onChange={(val)=>this.radialPositionChanged(val)} />
                            </div>
                        </React.Fragment>
                    )}
                </div>
