...
export function getBackground(background:Background):{
    background?: string,
...
export function getBackgroundGradient(config:GradientColor) {
    ...
    const preSizes = Object.keys(RadialGradientSizeType).map(key=>RadialGradientSizeType[key]);
    if(stops.length>0) {
        if(config.type === GradientType.linear) {
            props = {
                background: `linear-gradient(${getUnitValue(config.param.angle)},${stops})`
            };
        } else {
            let gradDef:string[] = [];
            ...
            let hasSize = false;
            if(preSizes.indexOf(gradientParams.size.radius.unit)!==-1) {
            ...
            if(gradientParams.position.first.value!=='' && gradientParams.position.second.value!=='') {
                gradDef.push('at');
...
export function getFontStyle(font:FontWithVariants, triggerLoad:boolean = true) {
    if(triggerLoad) {
