...
export interface DateRangePickerProps extends StandardProps<{}, DateRangePickerClassKey> {
    range:DateRange
...
class Component extends React.PureComponent<DateRangePickerProps, State> {
    ...
    protected rangeToString(range:DateRange) {
        return `${moment(range.startDate).format('MMMM DD, YYYY')} - ${moment(range.endDate).format('MMMM DD, YYYY')}`;
    }
        ...
        return (
            <ClickAwayListener onClickAway={()=>this.setState({open: false})}>
                <div>
                    ...
                    <Popper placement="bottom-end" style={{ zIndex: 1 }}
                                ...
                                <Paper>
                                    <div>
                                        ...
                                        <div className={classes.bottomContainer}>
                                            <Typography className={classes.datePreview}>
                                                {this.rangeToString(this.state.internalRange)}
                                            </Typography>
                                            <AppButton onClick={()=>this.setState({open: false})} className={classes.actionButton}>Cancel</AppButton>
                                            <AppButton onClick={this.onApply} className={classes.actionButton}>Apply</AppButton>
                                        </div>
                                    </div>
                                </Paper>
                            </Fade>
